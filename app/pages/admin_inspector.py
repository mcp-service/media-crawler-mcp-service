...
def render_admin_inspector() -> HTMLResponse:
    header = (
        create_logo(None, "MCP Tools Inspector") 
        + "<h1>工具调试台</h1>"
        + create_info_box("浏览并调试已注册的 MCP 工具，直接向对应 HTTP 接口提交请求，实时查看返回数据")
    )
    nav = render_nav()
    ...
    return build_page("".join(parts), title="MCP 调试台 · MediaCrawler MCP Service")