...
from app.core.crawler.proxy import IpCache, IpInfoModel, ProxyProvider
from app.core.crawler.proxy.types import ProviderNameEnum
from app.core.crawler.tools import utils
...
def parse_kuaidaili_proxy(proxy_info: str) -> KuaidailiProxyModel:
    ...
    if not match or not match.groups():
        raise ValueError("Can not parse kuaidaili proxy info")
    ...
    async def get_proxy(self, num: int) -> List[IpInfoModel]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code != 200:
                utils.logger.error(
                    "[KuaiDaiLiProxy.get_proxies] status code not 200 and response.txt:%s",
                    response.text,
                )
            ...
            if ip_response.get("code") != 0:
                utils.logger.error(
                    "[KuaiDaiLiProxy.get_proxies] code not 0 and msg:%s",
                    ip_response.get("msg"),
                )
